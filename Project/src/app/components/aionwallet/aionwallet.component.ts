...
export class AionwalletComponent implements  OnInit, OnDestroy {
  ...
  deposit() {
    ...
    var wxethaddress = '0xa0d436c9e6b53b1b3267adbd0557765c9fa0ab5bd2096fb3e0ee727d1d2d1ba6';
    self.instanceMyTokenContract=new web3Instance.eth.Contract(Constants.EtherTokenAbi,wxethaddress,{from:sessionStorage.getItem('walletAddress'),gasLimit:550000})
    ...
    //const conn=new web3Instance.eth.Contract(Constants.EtherTokenAbi,this.wxethaddress,{from:sessionStorage.getItem('walletAddress'),gasLimit:550000})
    console.log(this.web3,self.instanceMyTokenContract);
    const contractFunction = self.instanceMyTokenContract.methods.deposit(userAccount);
    const functionAbi = contractFunction.encodeABI();
    let estimatedGas;
    let nonce;
    let convertedAmount = this.amount*1000000000000000000;
    console.log("Getting gas estimate");
    web3Instance.eth.getTransactionCount(sessionStorage.getItem('walletAddress')).then(_nonce => {
     nonce = _nonce;
     console.log("Nonce: " + nonce);
     const txParams = {
      gas:999999,
      to:Constants.EtherTokenAddress,
      data: functionAbi,
  ...
  var privateKey='0xe2a2bdb97d9c2ab0e1974e6e0314372e28376b96ca75eace3d01f266e8416b09e6fb9e2108e5862e329cab6797a769579fc7f84b6bb4019598ef9ba099601804';
    web3Instance.eth.accounts.signTransaction(txParams,privateKey,function(err,res){
 console.log(res)
    // const serializedTx = tx.serialize();
console.log(web3Instance.eth.sendSignedTransaction);
 web3Instance.eth.sendSignedTransaction(res.rawTransaction).on('receipt', receipt => {
       console.log(receipt);
        self.instanceMyTokenContract.methods.balanceOf(sessionStorage.getItem("walletAddress")).call().then(function (result) {
          ...
          web3Instance.eth.getBalance(sessionStorage.getItem("walletAddress")).then( res => {
           this.wxETHBalance = (res/1000000000000000000).toFixed(4)
          });
